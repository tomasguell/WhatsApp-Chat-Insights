...
class Sender(models.Model):
    ...
    def __str__(self):
        return f"{self.Name} - Creado por {self.User} "  # {self.Chat.Title}"
...
class Message(models.Model):
    ...
    def __str__(self):
        return f"{self.Sender.Name} - {self.Date} {self.Time}"
...
def populate_senders(sender, instance, created, **kwargs):
    if created:
        ...
        for remitente in remitentes_unicos:
            sender, created = Sender.objects.get_or_create(Name=remitente, User=user)
            # print(sender)
            if created:
                print("LLEGE A LA CONDICION")
                sender_objects.append(sender)
                sender.save()
                print(f"{sender.id}{sender.Name} - {sender.User}")
